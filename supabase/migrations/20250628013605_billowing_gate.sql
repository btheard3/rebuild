...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
    CREATE TYPE user_role AS ENUM ('admin', 'coordinator', 'field_agent', 'viewer');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'case_status') THEN
    CREATE TYPE case_status AS ENUM ('open', 'active', 'resolved', 'closed');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'team_status') THEN
    CREATE TYPE team_status AS ENUM ('available', 'assigned', 'unavailable');
  END IF;
END $$;

-- Create profiles table with consistent column naming
CREATE TABLE IF NOT EXISTS profiles (
  id uuid PRIMARY KEY REFERENCES auth.users ON DELETE CASCADE,
  email text UNIQUE NOT NULL,
  full_name text,
  user_role user_role DEFAULT 'viewer',  -- Using user_role consistently
  phone text,
  organization text,
...
CREATE POLICY "Coordinators and admins can create cases"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = auth.uid()
      AND user_role IN ('coordinator', 'admin')
    )
  );
...
CREATE POLICY "Field agents and above can create updates"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE id = auth.uid()
      AND user_role IN ('field_agent', 'coordinator', 'admin')
    )
  );
