...
const openai = new OpenAI({
  ...
  dangerouslyAllowBrowser: true, // Note: In production, use a secure backend route
});
...
export const openaiService = {
  async generateScript(journalEntry: string, mood?: string): Promise<string> {
    try {
      ...
      return response.choices[0]?.message.content?.trim() || 'Unable to generate script at this time.';
    } catch (error) {
